...
export class sendTokenTranformService {
    constructor() {
    ...
    async sendToken(
        ...
        try {
            ...
            console.log('Bắt đầu quá trình chuyển token...');
            ...
            // Tạo hoặc lấy token account của người gửi (owner)
            ...
            console.log('Chuyển token thành công! Chữ ký giao dịch:', signature);
            ...
            console.error('Lỗi trong quá trình chuyển token:', error);
            if (error.logs) {
                console.error('Log giao dịch:', error.logs);
            }
            throw new Error(`Chuyển token thất bại: ${error.message}`);
        }
    }
